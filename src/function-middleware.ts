 ...
 */
export function runServerlessFunc(handler: Handler): (req: Request, res: Response, next: NextFunction) => void {
  return function runHandler(req: Request, res: Response, next: NextFunction): void {
    ...
    if (apiGatewayEvent) {
      ...
      if (handler.length <= 2) {
        ...
        handler(apiGatewayEvent, context, (err, result) => {
          if (err) {
            next(err);
          } else {
            (res as unknown as Record<string, unknown>).apiGatewayProxyResult = result;
